...
void bobcat_hidden(int abc[], int x[]);
...
int twoBytes_to_int ( char a, char b )
{
	int arr1 = (int)a;
	int arr2 = (int)b;
	return arr1 | (arr2 >> 8);
...
static PyObject* bobcat(PyObject* self, PyObject* args)
{
	char *msg_block;
	int abc[3];
	
	if(!PyArg_ParseTuple (args, "s", &msg_block )) 
		return NULL;
	if(strlen(msg_block) != 16)//it take 128 bits = 16 bytes * 8 bits
	{
		return NULL;
	}
	...
	int x [8];
//return Py_BuildValue("reached here");

	for(int i=0;i <8; i++)
	{
		x[i] = twoBytes_to_int(msg_block[i*2],msg_block[i*2+1]);
	}
	...
	bobcat_hidden(abc, &x[0]);

	PyObject * python_val = Py_BuildValue("[ii]", abc[0], abc[1], abc[2]);
	return python_val; 
}



static PyObject* bobcatIV(PyObject* self, PyObject* args)
{
	int abc[3];	
	int in[8];

	if(!PyArg_ParseTuple (args, "(iii)(iiiiiiii)", &abc[0], &abc[1], &abc[2], &in[0], &in[1], &in[2], &in[3], &in[4], &in[5] , &in[6], &in[7])) 
		return NULL;
...
//return Py_BuildValue("reached here");
	...
	bobcat_hidden(abc, &in[0]);


	PyObject *rslt = PyTuple_New(3);
	PyTuple_SetItem(rslt, 0,Py_BuildValue("i", abc[0]) );
	PyTuple_SetItem(rslt, 1,Py_BuildValue("i", abc[1]) );
	PyTuple_SetItem(rslt, 2,Py_BuildValue("i", abc[2]) );
	return rslt;
	//PyObject * python_val = Py_BuildValue("[iii]", abc[0], abc[1], abc[2]);
	//return python_val; 
}

static PyMethodDef myMethods[] = {
{"bobcatIV", bobcatIV, METH_VARARGS, "Calculate the hash_value of the bobcat hash function"},
{NULL,NULL,0,NULL}
};

static struct PyModuleDef bobModule ={
...
PyMODINIT_FUNC PyInit_bobcatModule (void)
{
	return PyModule_Create(&bobModule);
}
